...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TYPE "public"."enum_projects_status" AS ENUM('idea', 'in_progress', 'active', 'closed');
  CREATE TYPE "public"."enum_skills_icon" AS ENUM('code', 'design', 'product', 'marketing', 'analytics', 'leadership', 'strategy', 'fund-raising');
  ...
  CREATE TABLE IF NOT EXISTS "pages_hero_links" (
  	...
  	"link_appearance" "enum_pages_hero_links_link_appearance" DEFAULT 'link'
  );
  ...
  CREATE TABLE IF NOT EXISTS "pages_blocks_content_columns" (
  	...
  	"link_url" varchar,
  	"link_label" varchar,
  	"link_appearance" "enum_pages_blocks_content_columns_link_appearance" DEFAULT 'link'
  );
  ...
  CREATE TABLE IF NOT EXISTS "pages_blocks_skill_block" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"_path" text NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS "projects" (
  	...
  	"status" "enum_projects_status" DEFAULT 'idea' NOT NULL,
  	...
  	"project_link_label" varchar DEFAULT 'Visit Website' NOT NULL,
  	"project_link_url" varchar NOT NULL,
  	"slug" varchar,
  	"slug_lock" boolean DEFAULT true,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  ...
  DO $$ BEGIN
   ALTER TABLE "pages_blocks_skill_block" ADD CONSTRAINT "pages_blocks_skill_block_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."pages"("id") ON DELETE cascade ON UPDATE no action;
  EXCEPTION
   WHEN duplicate_object THEN null;
  END $$;
  ...
  CREATE INDEX IF NOT EXISTS "pages_blocks_skill_block_order_idx" ON "pages_blocks_skill_block" USING btree ("_order");
  CREATE INDEX IF NOT EXISTS "pages_blocks_skill_block_parent_id_idx" ON "pages_blocks_skill_block" USING btree ("_parent_id");
  CREATE INDEX IF NOT EXISTS "pages_blocks_skill_block_path_idx" ON "pages_blocks_skill_block" USING btree ("_path");
  ...
  CREATE INDEX IF NOT EXISTS "technologies_slug_idx" ON "technologies" USING btree ("slug");
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS "skills_title_idx" ON "skills" USING btree ("title");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "pages_blocks_skill_block" CASCADE;
